...
type CalendarDialogProps = DayPickerSingleProps & {
  className?: string
  buttonVariant?: React.ComponentProps<typeof Button>['variant']
}
...
export function SingleCalendarDialog({
  className,
  ...props
}: CalendarDialogProps) {
  ...
  const handleSelect: DayPickerSingleProps['onSelect'] = (
    selected,
    selectedDay,
    modifiers,
    event
  ) => {
    onSelect?.(selected, selectedDay, modifiers, event)
